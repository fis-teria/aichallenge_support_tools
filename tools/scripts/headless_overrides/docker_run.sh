...
if ! ln -sfn "${PWD}/${LOG_FILE}" output/latest/docker_run.log; then
    echo "[WARN] Failed to update output/latest/docker_run.log symlink." >&2
    echo "[WARN] Check output/latest ownership/permissions. The run log will still be written to: ${LOG_FILE}" >&2
fi
